...
# ==============================================================================
"""Combined worker that handles both detokenization and tokenizer management."""
...
class CombinedWorker:
    ...
    def __init__(
        ...
        # Decode status tracking (shared between detokenizer and manager)
        self.decode_status = LimitedCapacityDict(capacity=DETOKENIZER_MAX_STATES)

        # Request states (for TokenizerManager functionality)
        self.rid_to_state = {}
        ...
        self.performance_stats = {
            ...
            "enable_batch_logging": getattr(
                server_args, "enable_tokenizer_manager_batch_logging", False
            ),
        }

        logger.info(f"🔌 CombinedWorker {self.worker_id} - 🚀 Initialized successfully")

    def run(self):
        """Run the combined worker."""
        start_time = time.time()
        logger.info(f"🔌 CombinedWorker {self.worker_id} - 🚀 Starting...")
        ...
        try:
            ...
            logger.error(
                f"🔌 CombinedWorker {self.worker_id} - ❌ Failed after {total_time:.2f}s: {e}"
            )
            raise
        finally:
            total_time = time.time() - start_time
            logger.info(
                f"🔌 CombinedWorker {self.worker_id} - 🛑 Stopped after {total_time:.2f}s"
            )

    def event_loop(self):
        """The event loop that handles requests with zero IPC overhead."""
        while True:
            start_time = time.time()
            ...
            # Process the request (detokenization + management in one step)
            process_start = time.time()
            output = self._process_request(recv_obj)
            process_time = time.time() - process_start

            # Send result back to scheduler for direct client communication
            ...
            if self.performance_stats["enable_worker_logging"]:
                total_time = time.time() - start_time
                logger.info(
                    f"🔌 CombinedWorker {self.worker_id} - ✅ Completed cycle in {total_time:.4f}s:\n"
    ...
    def _process_request(self, recv_obj):
        """Process request with zero IPC overhead - sends responses back to scheduler."""
        if isinstance(recv_obj, BatchTokenIDOut):
            ...
            logger.error(
                f"🔌 CombinedWorker {self.worker_id} - Unknown request type: {type(recv_obj)}"
            )
            return recv_obj

    def _handle_batch_token_id_out(self, recv_obj: BatchTokenIDOut):
        """Handle BatchTokenIDOut with integrated detokenization and management."""
        start_time = time.time()
        bs = len(recv_obj.rids)

        if self.performance_stats["enable_worker_logging"]:
            logger.info(
                f"🔌 CombinedWorker {self.worker_id} - 🔤 Processing BatchTokenIDOut: {bs} requests"
            )
        ...
        if self.performance_stats["enable_worker_logging"]:
            logger.info(
                f"🔌 CombinedWorker {self.worker_id} - ✅ BatchTokenIDOut completed in {total_time:.4f}s"
            )

        # Return BatchStrOut directly (no intermediate IPC)
        return BatchStrOut(
            rids=recv_obj.rids,
    ...
    def _handle_batch_embedding_out(self, recv_obj: BatchEmbeddingOut):
        ...
        if self.performance_stats["enable_worker_logging"]:
            logger.info(
                f"🔌 CombinedWorker {self.worker_id} - 📊 Processing BatchEmbeddingOut: {len(recv_obj.rids)} requests"
            )
        return recv_obj
...
def run_combined_worker_process(
    server_args: ServerArgs,
    port_args: PortArgs,
    worker_id: int = 0,
):
    """Run CombinedWorker in a separate process."""
    import psutil
    import setproctitle

    setproctitle.setproctitle(f"sglang::combined_worker_{worker_id}")

    try:
        worker = CombinedWorker(server_args, port_args, worker_id)
        worker.run()
    except Exception as e:
        logger.error(f"🔌 CombinedWorker {worker_id} failed: {e}")
        raise
